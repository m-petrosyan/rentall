<template>
    <section class="p-3 sm:p-5 h-screen">
        <h2>Kits</h2>
        <div v-if="kits && !loading" class="mx-auto mt-10 max-w-screen-2xl">
                        ...
                        <tr v-for="kit in kits.data" :key="kit.id"
                            ...
                            <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                <div class="flex items-center space-x-4">
                                    <router-link :to="{name:'db-kitupdate',params:{id: kit.id}}" type="button"
                                                 class="py-2 px-3 text-center focus:outline-none text-white bg-grey-blue rounded">
                                        Edit
                                    </router-link>
                                    <button type="button" @click="deleteModal(kit.id)"
...
export default {
    ...
    methods: {
        ...
        deleteData() {
            return this.deleteKit(this.daleteId)
                .then(() => {
                    this.daleteId = null
                    this.getData()
                })
